...
parser.add_argument('-f','--frame_length', default=30,help='frame length sent to VAD')
...
for sesspath in sesslist: 
    ...
    if args.export_seg_audio: 
        print('Exporting segmented audio...')
        segDir = os.path.join(sesspath, 'VADsegments')
        blkDir = os.path.join(sesspath, 'VADblocks')
    ...
    frames = frame_generator(args.frame_length, sess_audio.raw_data, sample_rate)
    frames = list(frames)
    segment_generator = vad_collector(sample_rate,args.frame_length,int(args.win_length), vad, frames, args.min_seg_dur)
