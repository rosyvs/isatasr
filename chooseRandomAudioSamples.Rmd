...
catalog_file <- '../Data File Catalog_2022-03-17.xlsx'
...
for (k in 1:opts.Niter){
  ...
  for (t in 1:length(teacher_list)){
    ...
    if(length(vsheet)>1){vsheet=vsheet[-1]}
    if(length(asheet)>1){asheet=asheet[-1]}
    if (has_V){
      video <- read.xlsx(catalog_file, sheet=vsheet, detectDates=T,na.strings = "NA")
      # video[video=='NA'] <- NA
      names(video) <- make.names(names(video))
      video <- video %>% rename_at(vars(grep("Useable",names(.))), ~'useable')
      video <- video %>% filter(useable==1)
    ...
    if (has_A){
      audio <- read.xlsx(catalog_file, sheet=asheet,detectDates=T,na.strings = "NA")
    ...
    for (s in 1:nrow(class_students)){ # loop over students in this class
      ...
      if (!studentID %in% student_list){
        print(paste0('Student already appeared in a selected video: ', studentID))
        next}
      
      # check whether to use video or audio
      if (has_V & class_students$video_consent[s]){media <- video }else if(has_A & class_students$audio_consent[s]){media<-audio}else{print(paste('no consented media for Student ',s)) 
        next}
      
      # choose a media file in which the student appears
            media <- media %>% filter(grepl( studentID,listStudentID))
     ...
     thisrow <- thisrow %>% select(c(Teacher, Period, Lesson.., Name.of.File, URL.to.File, Student.IDs, contains('sample_')))
...
widesample %>% write.csv(paste0('../annotation/samples_wide_5inCenter_2022-03-17_',k,'.csv'))
