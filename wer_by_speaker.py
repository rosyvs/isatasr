...
asrType = 'asr_segwise'
transcriptType = 'transcripts_segwise'
by_codec = True # produce summary of WER by codec? Must be final field in sessname
...
for sesspath in sesslist: 
    ...
    utt_labels = pd.read_csv(os.path.join(sesspath, f'utt_labels_{sessname}.csv'))

    # get list of segments
    seglist = utt_labels['segment'].tolist()

    seg_data = []
    # loop over segments
    aligned_segwise=[]
    for s in seglist:
        this_utt_labels = utt_labels[utt_labels['segment']==s].reset_index()
        ...
        speaker = this_utt_labels['speaker'].item()
...
if by_codec:
    ...
    all_by_speakerXcodec = all_sess_seg_wer.groupby(['speaker_type','codec'],sort=False,as_index=False).apply(wer_df_summary).reset_index()

all_by_sess = all_sess_seg_wer.groupby('session',sort=False,as_index=False).apply(wer_df_summary).reset_index()

all_by_speaker = all_sess_seg_wer.groupby('speaker_type',sort=False,as_index=False).apply(wer_df_summary).reset_index()
all_by_sessXspeaker = all_sess_seg_wer.groupby(['session','speaker_type'],sort=False,as_index=False).apply(wer_df_summary).reset_index()

#pd.concat(all_sess_seg_wer).to_csv( f'results/sesswise_wer_{verstr}.csv',index=False, float_format='%.3f') 
with pd.ExcelWriter(f'results/sesswise_wer_bygroups_{ctlfile}_cf_{transcriptType}.xlsx',engine='xlsxwriter',
                        engine_kwargs={'options': {'strings_to_numbers': True}}) as writer:  
    all_by_sess.to_excel(writer, sheet_name='by_session')
    all_by_speaker.to_excel(writer, sheet_name='by_speakerType')
    all_by_sessXspeaker.to_excel(writer, sheet_name='by_sessXspeakerType')
